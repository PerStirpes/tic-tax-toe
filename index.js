window.onload = function() {
  ...
  function checkRow(box1, box2, box3, player) {
    if (
      selectBox(box1) === player &&
      selectBox(box2) === player &&
      selectBox(box3) === player
    ) {
      gameOver = true
    }
    if (gameOver) {
  ...
  const selectBox = box => document.getElementById(box).innerText

  //changes text color of winning boxes to green
  const winColor = box => (document.getElementById(box).style.color = '#228B22')
